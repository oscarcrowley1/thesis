...
for from_point in range(num_points):
    for to_point in range(num_points):
        ...
        # either configure the elif to set impossible routes to inf and else to calcualte distances
        # OR else calulates possible routes and the rest are set to inf
                ...
                (to_point in junction1 and from_point in junction1) or \
                    (to_point in junction2 and from_point in junction2) or \
                        (to_point in junction3 and from_point in junction3) or \
                            (to_point in junction4 and from_point in junction4):
                        distance = inf
...
np.save("interpret_csv_bravoplus/distance_mat_bravoplus", adj_array)
...
plt.colorbar(label="Adjacency (no units)")
plt.show()
...
np.save("interpret_csv_bravoplus/adj_mat_bravoplus", kernelised_adj)

f = open("interpret_csv_bravoplus/adj_info.txt", "w")
info_string = "Epsilon:\t" + str(epsilon) + "\nDelta Squared:\t" + str(delta_squared) + "\nUses these distances\n" + str(coord_array) + "\nAdjacency Matrix:\n" + str(kernelised_adj)
f.write(info_string)
f.close()


if os.path.isfile("interpret_csv_bravoplus/node_values_bravoplus.npy") and os.path.isfile("interpret_csv_bravoplus/nv_info.txt"):
